...
export default {
    ...
    methods: {
        async getFoldersData() {
            ...
            await axios.get('/api/folders/doramas')
                .then(response => {
...
<template>
    <div class="sticky top-20 w-72 bg-blackSimple shadow-[0_8px_6px_-4px_rgb(124,58,237)]">
        <div v-if="!dataFoldersLoading">
            ...
            >
                ...
                <span class="w-full h-full text-xl flex items-center justify-between truncate group-hover:bg-gray-100 group-hover:text-black">
                    ...
                    <span class="px-4">
                        {{ dataFolder.favorites_doramas_user_count }}
                    </span>
                </span>
            </label>
        </div>
        <div v-else class="w-full h-80 flex items-center justify-center">
            <LoadingSvg classes="size-20 fill-violet-500"/>
        </div>
    </div>
</template>
