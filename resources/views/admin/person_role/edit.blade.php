@extends('admin.admin')
@section('title', config('app.name') . ' - Редактирование Роли для персон')
@section('content')
        ...
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
            <div class="relative overflow-x-auto shadow-md">
                <div class="p-5 text-xl font-semibold text-left rtl:text-right text-gray-900 bg-white dark:text-white dark:bg-gray-800">
                    Редактирование роли для персон
                </div>
                <form action="{{ route('admin.person-role.update', $personRole->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="grid gap-6 mb-6 md:grid-cols-3">
                        <div>
                            ...
                            <input id="name"
                                   type="text"
                                   name="name"
                                   value="{{ old('name') ?? $personRole->name }}"
                            ...
                            <input id="slug"
                                   type="text"
                                   name="slug"
                                   value="{{ old('slug') ?? $personRole->slug }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                   required
                            />
                        </div>
