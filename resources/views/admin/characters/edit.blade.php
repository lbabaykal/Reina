@extends('admin.admin')
@section('title', config('app.name') . ' - Редактирование персонажа')
@section('content')
        ...
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
            <div class="relative overflow-x-auto shadow-md">
                <div class="p-5 text-xl font-semibold text-left rtl:text-right text-gray-900 bg-white dark:text-white dark:bg-gray-800">
                    Редактирование персонажа - {{ $character->full_name_ru }}
                    <form action="{{ route('admin.characters.destroy', $character->slug) }}" method="POST" class="pl-10 inline-block">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-2 py-1 bg-red-400 hover:bg-red-500 rounded-sm text-white">Удалить</button>
                    </form>
                </div>
                <form action="{{ route('admin.characters.update', $character->slug) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div class="grid gap-6 mb-6 md:grid-cols-3">
                        <div>
                            ...
                            <input id="full_name_org"
                                   type="text"
                                   name="full_name_org"
                                   value="{{ old('full_name_org') ?? $character->full_name_org }}"
                        ...
                        <div>
                            ...
                            <input id="full_name_ru"
                                   type="text"
                                   name="full_name_ru"
                                   value="{{ old('full_name_ru') ?? $character->full_name_ru }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                   required
                            />
                        </div>
                        <div>
                            ...
                            <input id="full_name_en"
                                   type="text"
                                   name="full_name_en"
                                   value="{{ old('full_name_en') ?? $character->full_name_en }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                   required
                            />
                        </div>
